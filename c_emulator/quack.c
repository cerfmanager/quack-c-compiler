...
Quompiler
TODO: either rewrite this in rust or add rewrite the c to include
...
static uint8_t io_read8(uint16_t addr) {
    switch (addr) {
        ...
        case IO_KEY:
            ...
            break;
        default:
            die("Trying to access non-mapped IO address range! This should be impossible!");
            break;
    }
    return -1; // this pleases the compiler
}
...
static void cpu_step(cpu_t *cpu, int debug) {
    ...
    switch (in.op) {
        ...
        case OP_CMPW:
            check_reg(in.ra);
            check_reg(in.b2);
            cpu->zf = cpu->r[in.ra] == cpu->r[in.b2];
            cpu->pc += 4;
            break;
        case OP_JMP:
        ...
        case OP_MRMOVB:
            check_reg(in.ra);
            mem_addr = u16_from_le(in.b2, in.b3);
            if (mem_addr >= IO_START && mem_addr <= IO_END) {
                cpu->r[in.ra] = io_read8(mem_addr);
            }
            else {
        ...
        case OP_RMMOVB:
            ...
            mem_addr = u16_from_le(in.b2, in.b3);
            if (mem_addr >= IO_START && mem_addr <= IO_END) {
                io_write8(mem_addr, cpu->r[in.ra] & 0xFF);
            }
            else {
                mem_write8(mem_addr, cpu->r[in.ra] & 0xFF);
            }
            cpu->pc += 4;
            break;
        case OP_MRMOVBR:
